...
const PointsTable = () => {
  ...
  customersData.map((user) => {user.purchasePoints = calcPts(user.purchaseTotal)})

  const sumUserMonthPts = (userId, month) => {
    let points = 0;
    const userMonth = customersData.filter((user) => user.userId == userId && user.purchaseDate.slice(0, 2) == month)
...
const createCustomerPtsData = () => {
  let customerPtsData = [];
  for(let i=1;i<=3;i++){
  customerPtsData.push({userId:i, janPts: sumUserMonthPts(i, "01"), febPts: sumUserMonthPts(i, "02"), marchPts: sumUserMonthPts(i, "03"), totalPts: sumUserTotalPts(i)})
  }
  return customerPtsData
}

const customerPtsData = createCustomerPtsData();
console.log(`customerPtsData`, customerPtsData)
